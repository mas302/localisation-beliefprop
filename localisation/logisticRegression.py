...
encodedParameters['networkLat'][midRows:rowCount] = df['gps_lat_mean'][midRows:rowCount] + np.random.rand(rowCount- midRows)

encodedParameters['networkLon'] = np.nan
encodedParameters['networkLon'][:midRows] = df['gps_long_mean'][:midRows]
encodedParameters['networkLon'][midRows:rowCount] = df['gps_long_mean'][midRows:rowCount] + np.random.rand(rowCount- midRows)
...
logreg = LogisticRegression(solver = 'saga', penalty= 'l2', random_state=16, verbose=1)
